...
int MSDKVideoEncoder::InitEncodeOnEncoderThread(
  ...
  if (frame_rate_ > 0) {
    AVRational fps = av_d2q(frame_rate_, 65535);
      RTC_LOG(LS_ERROR) << fps.num << " -> " << fps.den;
    m_mfxEncParams.mfx.FrameInfo.FrameRateExtN = fps.num;
    m_mfxEncParams.mfx.FrameInfo.FrameRateExtD = fps.den;
  } else {
...
retry:
  ...
  encodedFrame._frameType = (bs.FrameType & MFX_FRAMETYPE_IDR || bs.FrameType & MFX_FRAMETYPE_xIDR)
